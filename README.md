...
> ⚠️ **구글 캘린더 연동 안내**: 현재 Google 인증 심사 중입니다. 로그인 시 "Google에서 확인하지 않은 앱" 경고가 표시될 수 있습니다. "고급" → "이동(안전하지 않음)"을 클릭하여 계속 진행할 수 있습니다. 읽기 권한만 요청하므로 안전합니다.

## 🛠️ 기술 스택

- **Frontend**: HTML5, CSS3, Vanilla JavaScript
- **PDF 생성**: jsPDF + html2canvas
- **Google API**: Google Calendar API, Google Identity Services
- **호스팅**: GitHub Pages
...
1. **로그인**: "구글 캘린더 연동" 버튼 클릭
2. Google 계정으로 로그인 (캘린더 읽기 권한만 요청)
...
- **Google 캘린더**: 읽기 권한만 요청하며, 캘린더 데이터는 로컬에서만 처리됩니다
